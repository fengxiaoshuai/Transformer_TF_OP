...
NMT::Encoder::Encoder(const size_t& head_num,
...
*/
{
	...
	for(int i=0; i<batch_size; i++)
	{
		...
		float* begin = weight_language + hidden_num * target_language_id[i];
...
vector<float> NMT::Encoder::Encode(vector<int>& input, const size_t& batch_size, const size_t& length, vector<int>& mask, vector<int>& language_id)
{
	...
	for (int i = 0; i < layer_num; i++)
	{
		...
		LayerPreprocess(attention_out, batch_size, length, weight[i][0], weight[i][1]);
		Attention(attention_out.data(), batch_size, length, weight[i][2], weight[i][3], weight[i][4], weight[i][12], weight[i][13], weight[i][5], bias.data(), attention_out.data());
		...
		LayerPreprocess(attention_out, batch_size, length, weight[i][6], weight[i][7]);
		FeedForward(attention_out, ffn_out_1, batch_size, length, filter_size, weight[i][8], weight[i][9], "relu");
		FeedForward(ffn_out_1, attention_out, batch_size, length, hidden_num, weight[i][10], weight[i][11], "none");
		SetZero(batch_size, length, attention_out.data(), mask.data());
		LayerPostprocess(embedding_word, attention_out);
                
	};
	LayerPreprocess(embedding_word, batch_size, length, weight_scale, weight_bias);
